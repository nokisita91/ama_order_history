...
include Selenium::WebDriver::Error
  ...
  #   open_top_page
  #   open_login_page
  #   enter_email_address(account[:email])
  ...
  def login(account_mail, account_password)
    open_top_page
    open_login_page
    enter_email_address(account_mail)
    enter_password(account_password)
    wait_for_logged_in
  end
  ...
  def collect_ordered_items
    ...
    loop do
      collect_order_items_by_page(order_infos)
      begin
  ...
  def collect_order_items_by_page(order_infos)
